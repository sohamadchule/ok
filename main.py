from flask import Flask, render_template, request, redirect, url_for, flash, session, jsonify, make_response,flash
...
def doctor_dashboard():
    ...
    # Fetch appointments and records from DB
    appointments = Appointment.query.filter_by(doctor_id=doctor_id).all()
    records = PatientRecord.query.filter_by(doctor_id=doctor_id).order_by(PatientRecord.visit_date.desc()).limit(10).all()
    ...
    return render_template(
        ...
        records=records,
        now=datetime.now  # so you can call `now()` in Jinja
    )
...
@app.route('/doctor/search_patient', methods=['GET', 'POST'])
@doctor_login_required
def search_patient():
    if request.method == 'POST':
...
@app.route('/doctor/download_prescription/<int:record_id>')
@doctor_login_required
def download_prescription(record_id):
    record = PatientRecord.query.get_or_404(record_id)
    
    # Ensure the doctor can only access their own records
    if record.doctor_id != session['doctor_id']:
        flash('Unauthorized access', 'danger')
        return redirect(url_for('doctor_dashboard'))
    
    # Logic for generating and downloading prescription would go here
    # For now, we'll just redirect back with a message
    flash('Prescription downloaded successfully', 'success')
    return redirect(url_for('doctor_dashboard'))
...
def appointment():
    ...
    doctors = Doctor.query.all()
    return render_template('appointment/book.html', doctors=doctors , departments=x )
